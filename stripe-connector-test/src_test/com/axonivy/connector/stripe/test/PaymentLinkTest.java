...
public class PaymentLinkTest extends BaseTest {
	...
	void createPaymentLink_mockedApi_returnsMockedResponse(BpmClient bpmClient) {
		...
		assertThat(resultPaymentLink).isNotNull();
		assertThat(resultPaymentLink.getId()).isEqualTo(MOCK_PAYMENT_LINK_ID);
		assertThat(resultPaymentLink.getUrl()).isEqualTo(MOCK_PAYMENT_LINK_URL);
		assertThat(resultPaymentLink.isActive()).isTrue();
	}
	...
	void retrievePaymentLinkLineItems_mockedApi_returnsMockedResponse(BpmClient bpmClient) {
		...
		assertThat(resultLineItems).isNotNull();
		assertThat(resultLineItems.getData()).isNotEmpty();
		assertThat(resultLineItems.getData()).hasSize(2);
	}
	...
	void setPaymentLinkActive_deactivate_mockedApi_returnsMockedResponse(BpmClient bpmClient) {
		...
		assertThat(resultPaymentLink).isNotNull();
		assertThat(resultPaymentLink.getId()).isEqualTo(MOCK_PAYMENT_LINK_ID);
		assertThat(resultPaymentLink.isActive()).isFalse();
	}
